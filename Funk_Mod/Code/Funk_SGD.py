...
def pred_results(data, mean_rating, user_bias, item_bias, user_factor, item_factor):
    ...
    for i in range(num_data):
        user_id = data[i, 0]
        item_id = data[i, 1]
        pred_curr = mean_rating + user_bias[user_id] + item_bias[item_id] + np.dot(user_factor[user_id, :], item_factor[item_id, :])
...
def sgd_funk(train_data, val_data, num_users, num_items, num_factors, num_epochs, learning_rate, reg_param):
    ...
    for i in range(num_epochs):
        ...
        pred_val_ratings = pred_results(val_data, mean_rating, user_bias, item_bias, user_factor, item_factor)
